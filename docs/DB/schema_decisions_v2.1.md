...
- 상태: Working Update - Category Identity Migration 검증 전
- 대상 버전: ERD v2.1
- 기준일: 2026-09-07
- DBMS: MariaDB
- ORM: SQLAlchemy 2.x
...
- 변경 목적: 계층별 중복 표시명을 허용하면서 카테고리 식별자를 안정화

## v2.1 변경 요약

- ERD v2의 초기 동결 기록은 그대로 보존한다.
- `categories.category_name`의 전역 UNIQUE를 제거한다.
- `categories.category_code VARCHAR(50) NOT NULL UNIQUE`를 추가한다.
- `category_name`은 사용자에게 보여 주는 표시명, `category_code`는 시스템 식별용 안정 키로 구분한다.
- 서로 다른 부모 아래에서는 같은 표시명을 허용한다. 예: `음식 > 기타`, `게임 > 기타`.
- 이 문서는 목표 스키마를 정의한다. 실제 ORM과 Alembic 반영 여부는 대상 브랜치 최신 HEAD에서 별도로 검증한다.
...
* 카테고리의 시스템 식별에는 `category_code VARCHAR(50)`을 사용하고 전역 UNIQUE와 NOT NULL을 적용한다.
* `category_code`는 영문 대문자, 숫자, 밑줄로 구성한 UPPER_SNAKE_CASE를 사용한다.
* 대분류 코드는 짧은 도메인 코드로 정의한다. 예: `FOOD`, `GAME`.
* 세부분류 코드는 부모 맥락을 포함해 충돌 없이 정의한다. 예: `FOOD_OTHER`, `GAME_OTHER`.
* 표시명 변경만으로 `category_code`를 자동 변경하지 않는다. 코드 변경은 데이터 참조 영향을 검토하는 명시적 Migration으로 수행한다.
* 같은 부모 안의 중복 표시명 금지 여부는 Category Admin 기능을 설계할 때 별도 불변조건으로 확정한다. 현재 MVP에서는 관리 API가 없으므로 Seed/Migration 검증으로 방어한다.
...
현재 코드와 Migration을 확인한 뒤 다음 순서로 반영한다.

1. `category_code`를 우선 NULL 허용으로 추가한다.
2. 기존 카테고리 전체에 중복되지 않는 코드를 Backfill한다.
3. 누락과 중복이 없음을 검증한다.
4. `category_code`를 NOT NULL로 변경하고 UNIQUE 제약을 생성한다.
5. `category_name`의 기존 전역 UNIQUE 제약을 제거한다.
6. 서로 다른 부모 아래 같은 이름의 세부분류가 함께 저장되는 Integration Test를 추가한다.

운영 데이터가 존재하는 상태에서 `NOT NULL UNIQUE` 컬럼을 한 번에 추가하지 않는다. 실제 제약 이름과 Alembic Revision은 대상 브랜치의 현재 ORM 및 Migration을 확인해 결정한다.
